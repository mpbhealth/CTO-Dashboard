...
export default function APIStatus() {
  ...
  const getSeverityColor = (s: string) => {
    const colors = { 'critical': 'bg-red-100 text-red-800', 'warning': 'bg-amber-100 text-amber-800', 'info': 'bg-blue-100 text-blue-800' };
    return colors[s as keyof typeof colors] || 'bg-slate-100 text-slate-800';
  };

  const getIncidentStatusColor = (s: string) => {
    const colors = { 'resolved': 'bg-emerald-100 text-emerald-800', 'monitoring': 'bg-blue-100 text-blue-800', 'identified': 'bg-amber-100 text-amber-800', 'investigating': 'bg-orange-100 text-orange-800' };
    return colors[s as keyof typeof colors] || 'bg-slate-100 text-slate-800';
  };
  ...
  const ApiForm = ({ onSubmit, isEdit = false }: { onSubmit: (e: React.FormEvent) => void; isEdit?: boolean }) => (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="flex justify-end space-x-2">
        ...
        <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">{isEdit ? 'Update' : 'Add API'}</button>
      </div>
    </form>
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-3">
          ...
          <button onClick={() => setShowAddApi(true)} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors"><Plus className="h-4 w-4" /><span>Add API</span></button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {apis.map(api => (
          <div key={api.id}>
            {editApiId === api.id ? (
              <div className="bg-blue-50 p-6 rounded-xl shadow-sm border border-blue-300"><h3 className="text-lg font-semibold text-slate-900 mb-4">Edit API</h3>
                ...
                return (
                  <div key={inc.id}>
                    {editIncidentId === inc.id ? (
                      <div className="p-4 border-2 border-blue-300 rounded-lg bg-blue-50"><h4 className="font-medium text-slate-900 mb-3">Edit Incident</h4>
                      ...
                      <div className="flex items-start space-x-4 p-4 border border-slate-200 rounded-lg hover:bg-slate-50">
                        <div className={`w-3 h-3 rounded-full mt-2 ${inc.severity === 'critical' ? 'bg-red-500' : inc.severity === 'warning' ? 'bg-amber-500' : 'bg-blue-500'}`}></div>
                        ...
                        <div className="flex space-x-1">
                          <button onClick={() => { setIncidentForm({ api_id: inc.api_id, title: inc.title, description: inc.description, severity: inc.severity, status: inc.status, impact: inc.impact || '', resolution_notes: inc.resolution_notes || '' }); setEditIncidentId(inc.id); }} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="Edit"><Edit2 className="h-4 w-4" /></button>
