...
function DualDashboardContent() {
  ...
  const isSharedRoute = useMemo(() =>
    location.pathname.startsWith('/shared/') || location.pathname === '/presentation',
  [location.pathname]);
  ...
  const shouldShowCTOSidebar = useMemo(() => {
    ...
    // CEO users still need a sidebar on shared/presentation routes
    if (role === 'ceo') return isSharedRoute;
    return role === 'cto' || role === 'admin' || role === 'staff';
  }, [profileReady, isCEORoute, isAdminRoute, isSharedRoute, profile?.role]);
