...
export class ProductionDiagnostics {
  ...
  static testAssetLoading() {
    ...
    const hasStyles = Array.from(document.styleSheets).some(sheet => 
      sheet.href?.includes('assets/index-') || sheet.href?.includes('main.')
    );
    console.log('CSS loaded:', hasStyles);
    
    // Test if main JS loaded
    const hasMainJS = Array.from(document.scripts).some(script =>
      script.src?.includes('assets/index-') || script.src?.includes('main.')
    );
    ...
    const checkImage = (src: string) => {
      ...
      img.onerror = () => console.warn(`❌ Icon failed: ${src}`);
      img.src = src;
    };
...
if (import.meta.env.PROD) {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('🛠️ Debug tools available: diagnose(), clearAllCaches()');
  });
}